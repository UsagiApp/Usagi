...
class BackupRepository
	...
	constructor(
				...
				if (section in sections) {
					result +=
						when (section) {
							...
							// useless in our case
							BackupSection.HISTORY -> {
								input.readJsonArray<HistoryBackup>(serializer()).restoreToDb {
							...
							BackupSection.CATEGORIES -> {
								input.readJsonArray<CategoryBackup>(serializer()).restoreToDb {
							...
							BackupSection.BOOKMARKS -> {
								input.readJsonArray<BookmarkBackup>(serializer()).restoreToDb {
									upsertManga(it.manga)
							...
							BackupSection.SOURCES -> {
								input.readJsonArray<SourceBackup>(serializer()).restoreToDb {
									getSourcesDao().upsert(it.toEntity())
								}
							}

							BackupSection.SCROBBLING -> {
								input.readJsonArray<ScrobblingBackup>(serializer()).restoreToDb {
									getScrobblingDao().upsert(it.toEntity())
								}
							}

							BackupSection.STATS -> {
								input.readJsonArray<StatisticBackup>(serializer()).restoreToDb {
							...
							BackupSection.SAVED_FILTERS -> {
								input
									.readJsonArray<PersistableFilter>(serializer())
		...
		private suspend inline fun <T> Sequence<T>.restoreToDb(
			...
			fold(CompositeResult.EMPTY) { result, item ->
				result +
		...
		private suspend inline fun <T> Sequence<T>.restoreWithoutTransaction(
