...
class ExternalBackupStorage
	...
	constructor(
		...
		@CheckResult
		suspend fun delete(victim: BackupFile) =
			runInterruptible(Dispatchers.IO) {
		...
		private fun getRootOrThrow(): DocumentFile {
			val uri =
				checkNotNull(settings.periodicalBackupDirectory) {
