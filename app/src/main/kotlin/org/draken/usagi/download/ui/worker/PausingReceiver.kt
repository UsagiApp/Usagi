...
class PausingReceiver(
	...
	companion object {
		...
		fun createIntentFilter(id: UUID) =
			IntentFilter().apply {
		...
		fun getPauseIntent(
			context: Context,
			id: UUID,
		) = createIntent(context, id, ACTION_PAUSE)

		fun getResumeIntent(
			context: Context,
			id: UUID,
		) = createIntent(context, id, ACTION_RESUME)

		fun getSkipIntent(
			context: Context,
			id: UUID,
		) = createIntent(context, id, ACTION_SKIP)

		fun getSkipAllIntent(
			context: Context,
			id: UUID,
		) = createIntent(context, id, ACTION_SKIP_ALL)
