...
class DownloadWorker
	...
	constructor(
			...
			mangaLock.withLock(manga) {
				...
				try {
					...
					for ((chapterIndex, chapter) in chapters.withIndex()) {
						...
						}.withTicker(2L, TimeUnit.SECONDS)
							.collect { progress ->
								publishState(
									currentState.copy(
		...
		private suspend fun downloadFile(
			...
			return imageProxyInterceptor
				.interceptPageRequest(request, okHttp)
				.ensureSuccess()
				.use { response ->
					var file: File? = null
					try {
		...
		private suspend fun downloadPageFile(
			page: MangaPage,
			destination: File,
			...
			return repo
				.getPageResponse(page, okHttp, imageProxyInterceptor)
				.ensureSuccess()
				.use { response ->
					var file: File? = null
					try {
		...
		private fun File.createTempFile(ext: String?) =
			File(
				this,
				buildString {
		...
		private suspend fun getDoneChapters(manga: Manga) =
			runCatchingCancellable {
		...
		class Scheduler
			...
			constructor(
				...
				fun observeWorks(): Flow<List<WorkInfo>> =
					workManager
				...
				private fun createConstraints(allowMeteredNetwork: Boolean) =
					Constraints
						.Builder()
