...
sealed class FilterItem(
	...
	class CheckBox(
		...
		override fun equals(other: Any?) =
			other is CheckBox && other.path == path && other.title == title && other.isChecked == isChecked &&
				other.depth == depth

		override fun hashCode() = path.hashCode() * 31 + isChecked.hashCode()

		override fun getChangePayload(previousState: ListModel) = if (previousState is CheckBox && previousState.isChecked != isChecked) PAYLOAD_CHECKED_CHANGED else null
	}
	...
	class TriState(
		...
		override fun equals(other: Any?) = other is TriState && other.path == path && other.title == title && other.state == state && other.depth == depth

		override fun hashCode() = path.hashCode() * 31 + state

		override fun getChangePayload(previousState: ListModel) = if (previousState is TriState && previousState.state != state) PAYLOAD_CHECKED_CHANGED else null
	}
	...
	class Text(
		...
		override fun equals(other: Any?) = other is Text && other.path == path && other.title == title && other.value == value && other.depth == depth
	...
	class CheckBoxChips(
		...
		class Chip(
			...
			override fun equals(other: Any?) = other is Chip && other.path == path && other.title == title && other.checked == checked
		...
		override fun equals(other: Any?) = other is CheckBoxChips && other.path == path && other.chips == chips && other.depth == depth
	...
	class SortOption(
		...
		override fun areItemsTheSame(other: ListModel) = other is SortOption && other.path == path && other.optionIndex == optionIndex
		...
		override fun getChangePayload(previousState: ListModel) = if (previousState is SortOption && previousState.isAscending != isAscending) PAYLOAD_CHECKED_CHANGED else null
	}
}
