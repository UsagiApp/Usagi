...
class FilterHeaderFragment :
	...
	override fun onCreateViewBinding(
		...
		container: ViewGroup?,
	): FragmentFilterHeaderBinding = FragmentFilterHeaderBinding.inflate(inflater, container, false)

	override fun onViewBindingCreated(
		binding: FragmentFilterHeaderBinding,
		savedInstanceState: Bundle?,
	) {
		super.onViewBindingCreated(binding, savedInstanceState)
		binding.chipsTags.onChipClickListener = this
		binding.chipsTags.onChipCloseClickListener = this
		filterHeaderProducer
			.observeHeader(filter)
	...
	override fun onChipCloseClick(
		...
		when (data) {
			is String -> {
				if (data == filter.snapshot().listFilter.author) {
			...
			is ContentRating -> {
				filter.toggleContentRating(data, false)
			}

			is Demographic -> {
				filter.toggleDemographic(data, false)
			}

			is ContentType -> {
				filter.toggleContentType(data, false)
			}

			is MangaState -> {
				filter.toggleState(data, false)
			}
			...
			is Int -> {
				filter.setYear(YEAR_UNKNOWN)
			}

			is IntRange -> {
				filter.setYearRange(YEAR_UNKNOWN, YEAR_UNKNOWN)
			}
		}
	}
