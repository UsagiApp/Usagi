	...
	constructor(
		...
		private suspend fun MangaRepository.getFilter(
			...
			when (kind) {
				...
				SearchKind.AUTHOR -> {
					if (filterCapabilities.isAuthorSearchSupported) {
				...
				SearchKind.TAG -> {
					val tags =
						this@SearchV2Helper.dataRepository.findTags(this.source) +
		...
		private fun MutableList<Manga>.postFilter(
			query: String,
			kind: SearchKind,
		) {
		...
		private fun MutableList<Manga>.sortByRelevance(
			query: String,
			kind: SearchKind,
		) {
			when (kind) {
		...
		@Suppress("DEPRECATION")
		private fun Manga.matches(
			query: String,
			threshold: Float,
		): Boolean = matchesTitles(title, query, threshold) || matchesTitles(altTitle, query, threshold)

		private fun matchesTitles(
			a: String?,
			b: String?,
			threshold: Float,
		): Boolean = !a.isNullOrEmpty() && !b.isNullOrEmpty() && a.almostEquals(b, threshold)
