...
class MangaSearchRepository
	...
	constructor(
		...
		suspend fun getTagsSuggestion(
			query: String,
			limit: Int,
			source: MangaSource?,
		): List<MangaTag> =
			when {
				query.isNotEmpty() && source != null -> {
					db
						.getTagsDao()
				...
				query.isNotEmpty() -> {
					db.getTagsDao().findTags("%$query%", limit)
				}

				source != null -> {
					db.getTagsDao().findPopularTags(source.name, limit)
				}

				else -> {
					db.getTagsDao().findPopularTags(limit)
				}
			}.toMangaTagsList()
		...
		suspend fun getRareTags(
			source: MangaSource,
			limit: Int,
		): List<MangaTag> = db.getTagsDao().findRareTags(source.name, limit).toMangaTagsList()

		suspend fun getTopTags(
			source: MangaSource,
			limit: Int,
		): List<MangaTag> = db.getTagsDao().findPopularTags(source.name, limit).toMangaTagsList()
		...
		suspend fun getAuthors(
			source: MangaSource,
			limit: Int,
		): List<String> = db.getMangaDao().findAuthorsBySource(source.name, limit)
	}
