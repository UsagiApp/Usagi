...
class LocalMangaParser(
	...
	suspend fun getManga(withDetails: Boolean): LocalManga =
		runInterruptible(Dispatchers.IO) {
			(uri.resolveFsAndPath()).use { (fileSystem, rootPath) ->
				...
				if (mangaInfo != null) {
					val coverEntry: Path? =
						index.getCoverEntry()?.let { rootPath / it }?.takeIf {
	...
	suspend fun getMangaInfo(): Manga? =
		runInterruptible(Dispatchers.IO) {
			uri.resolveFsAndPath().use { (fileSystem, rootPath) ->
				val index = MangaIndex.read(fileSystem, rootPath / ENTRY_NAME_INDEX)
				index?.getMangaInfo()
						...
						openZip(file).use { zipFs ->
							zipFs.findFirstImageUri(Path.DIRECTORY_SEPARATOR.toPath())?.let { subUri ->
								val subPath =
									subUri.path
										.orEmpty()
										.removePrefix(uri.path.orEmpty())
										.replace(REGEX_PARENT_PATH_PREFIX, "")
								return@runCatchingCancellable uri
									.child(file, resolve = true)
	...
	private fun Path.userFriendlyName(): String =
		name
			.substringBeforeLast('.')
			.replace('_', ' ')
			.toTitleCase()
	...
	companion object {
		...
		@Blocking
		fun getOrNull(file: File): LocalMangaParser? =
			if ((file.isDirectory || file.isZipArchive) && file.canRead()) {
		...
		private fun Uri.resolveFsAndPath(): FsAndPath {
			...
			return when {
				...
				isFileUri() -> {
					...
					throw IllegalArgumentException("Unsupported uri $resolved")
				}
			}
		}

		private fun String.toRootedPath(): Path =
		...
		private fun String.fileNameToTitle() =
			substringBeforeLast('.')
				.replace('_', ' ')
