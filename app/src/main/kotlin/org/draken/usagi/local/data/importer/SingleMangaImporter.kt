...
class SingleMangaImporter
	...
	constructor(
		...
		suspend fun import(uri: Uri): LocalManga {
			val result =
				if (isDirectory(uri) || isPdfUri(contentResolver, uri)) {
					importDirectory(uri)
				} else {
					importFile(uri)
				}
		...
		private suspend fun importFile(uri: Uri): LocalManga =
		...
		private fun renderPdfPage(
			...
			try {
				...
				File(outDir, "%04d.jpg".format(index + 1))
					.outputStream()
		...
		private suspend fun DocumentFile.source() =
