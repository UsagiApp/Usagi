...
class LocalStorageManager
	...
	constructor(
		...
		suspend fun computeCacheSize(cache: CacheDir) =
			withContext(Dispatchers.IO) {
				getCacheDirs(cache.dir).sumOf { it.computeSize() }
			}

		suspend fun computeCacheSize() =
			withContext(Dispatchers.IO) {
				getCacheDirs().sumOf { it.computeSize() }
			}

		suspend fun computeStorageSize() =
			withContext(Dispatchers.IO) {
				getConfiguredStorageDirs().sumOf { it.computeSize() }
			}

		suspend fun computeAvailableSize() =
		...
		suspend fun resolveUri(uri: Uri): File =
			runInterruptible(Dispatchers.IO) {
				if (uri.isFileUri()) {
					uri.toFile()
				} else {
		...
		suspend fun setDirIsNoMedia(dir: File) =
			runInterruptible(Dispatchers.IO) {
		...
		suspend fun getDirectoryDisplayName(
			dir: File,
			isFullPath: Boolean,
		): String =
			runInterruptible(Dispatchers.IO) {
		...
		private fun getFallbackStorageDir(): File? =
			context.getExternalFilesDir(DIR_NAME) ?: File(context.filesDir, DIR_NAME).takeIf {
				it.exists() || it.mkdirs()
			}
