	...
	constructor(
		...
		fun inflatePreferences(): List<SettingsItem> {
			...
			preferenceManager.inflateTo(
				result,
				R.xml.pref_reader,
				listOf(context.getString(R.string.reader_settings)),
				ReaderPreviewSettingsFragment::class.java,
			)
			preferenceManager.inflateTo(
				result,
					...
					pref.inflateTo(
						...
						breadcrumbs =
							if (screenTitle.isNullOrEmpty() ||
								pref !is PreferenceScreen
							) {
								breadcrumbs
							} else {
								breadcrumbs + screenTitle
							},
						fragmentClass = fragmentClass,
					)
				} else {
