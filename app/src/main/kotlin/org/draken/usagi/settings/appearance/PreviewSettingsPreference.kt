...
class PreviewSettingsPreference
	@JvmOverloads
	constructor(
		context: Context,
			...
			rows.forEachIndexed { ri, widths ->
				...
				widths.forEachIndexed { ci, w ->
					row.addView(
						View(ctx).apply {
							layoutParams =
								LinearLayout.LayoutParams(w, chipH).also {
		...
		private fun buildModernContent(parent: LinearLayout) {
			...
			valueWidths.forEachIndexed { i, valWidth ->
				...
				row.addView(
					View(ctx).apply {
						layoutParams =
							LinearLayout.LayoutParams(valWidth, ctx.dp(6)).also {
		...
		private fun parseMode(name: String?): DetailsUiMode = DetailsUiMode.entries.firstOrNull { it.name == name } ?: DetailsUiMode.MODERN
