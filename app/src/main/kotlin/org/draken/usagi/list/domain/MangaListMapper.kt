...
class MangaListMapper
	...
	constructor(
		...
		suspend fun toListModelList(
			...
			ArrayList<MangaListModel>(manga.size).apply {
				toListModelList(
					destination = this,
					manga = manga,
					mode = mode,
		...
		suspend fun toFeedItem(logItem: TrackingLogItem) =
			FeedItem(
				id = logItem.id,
		...
		fun mapTags(tags: Collection<MangaTag>) =
			tags.map {
				ChipsView.ChipModel(
					tint = getTagTint(it),
					title = it.title,
		...
		private suspend fun isFavorite(
			mangaId: Long,
			@Options options: Int,
		): Boolean = options.isBadgeEnabled(FAVORITE) && favouritesRepository.isFavorite(mangaId)

		private suspend fun isSaved(
			mangaId: Long,
			@Options options: Int,
		): Boolean = options.isBadgeEnabled(SAVED) && mangaId in localMangaIndex

		@ColorRes
		private fun getTagTint(tag: MangaTag): Int =
			if (settings.isTagsWarningsEnabled && tag.title.lowercase() in dict) {
				R.color.warning
			} else {
				0
			}
		...
		private fun Int.isBadgeEnabled(
			@Options badge: Int,
		) = this and badge == badge
