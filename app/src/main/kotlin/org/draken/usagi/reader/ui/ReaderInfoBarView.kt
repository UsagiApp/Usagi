...
class ReaderInfoBarView
	...
	constructor(
		...
		override fun verifyDrawable(who: Drawable): Boolean = batteryIcons.any { it === who } || super.verifyDrawable(who)
		...
		private fun Canvas.drawTextOutline(
			text: String,
			x: Float,
			y: Float,
		) {
		...
		private fun getSystemUiDimensionOffset(
			name: String,
			fallback: Int = 0,
		): Int =
			runCatching {
