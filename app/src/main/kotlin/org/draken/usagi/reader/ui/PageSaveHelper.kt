...
class PageSaveHelper
	...
	constructor(
		...
		suspend fun save(tasks: Collection<Task>): Collection<Uri> =
			when (tasks.size) {
				0 -> emptySet()
		...
		private suspend fun saveImpl(task: Task): Uri {
			...
			val proposedName = task.getFileBaseName() + "." + getPageExtension(pageUrl, pageUri)
			val destination =
				getDefaultFileUri(proposedName)?.uri ?: run {
					val defaultUri =
						settings
							.getPagesSaveDir(context)
							?.uri
							?.buildUpon()
		...
		private suspend fun saveImpl(tasks: Collection<Task>): Collection<Uri> {
			...
			for (task in tasks) {
				...
				val mime =
					requireNotNull(MimeTypes.getMimeTypeFromExtension("_.$ext")) {
		...
		private suspend fun getPageExtension(
			url: Uri,
			fileUri: Uri,
		): String {
			val name =
		...
		private suspend fun getPageLoader() =
		...
		private fun getSource(uri: Uri): Source =
			when {
				...
				uri.isZipUri() -> {
					FileSystem.SYSTEM
						.openZip(uri.schemeSpecificPart.toPath())
						.source(requireNotNull(uri.fragment).toPath())
				}

				else -> {
					throw IllegalArgumentException("Bad uri $uri: unsupported scheme")
				}
			}

		private suspend fun copyImpl(
			source: Uri,
			destination: Uri,
		...
		private suspend fun getImageExtension(file: File): String? =
