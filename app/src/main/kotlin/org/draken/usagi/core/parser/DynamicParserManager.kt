...
class PluginClassLoader(
    ...
    override fun loadClass(name: String, resolve: Boolean): Class<*> {
        // 1. Force the JAR to delegate these shared API classes to Usagi (parent classloader).
        ...
        if (name == "org.koitharu.kotatsu.parsers.util.LinkResolver" ||
            ...
            (name.startsWith("org.koitharu.kotatsu.parsers.model.") && name != "org.koitharu.kotatsu.parsers.model.MangaParserSource") ||
            name.startsWith("org.koitharu.kotatsu.parsers.config.")
        ) {
            return super.loadClass(name, resolve)
        }

        // 2. Force the JAR to load these specific interfaces and internal implementations from itself.
...
object DynamicParserManager {
    ...
    fun loadParsersFromJar(context: Context, jarFile: File) {
        val cacheDir = context.getCodeCacheDir().absolutePath
    ...
    fun createParser(source: MangaSource, context: MangaLoaderContext): MangaParser {
        ...
        return Proxy.newProxyInstance(
            ...
            InvocationHandler { _, invokedMethod, args ->
