...
class FastScroller
	...
	constructor(
					...
					if (isEnabled) {
						when (newState) {
							...
							RecyclerView.SCROLL_STATE_IDLE -> {
								if (hideScrollbar && !binding.thumb.isSelected) {
		...
		fun setBubbleTextColor(
			@ColorInt color: Int,
		) = binding.bubble.setTextColor(color)
		...
		private fun TypedArray.getBubbleSize(
			@StyleableRes index: Int,
			defaultValue: BubbleSize,
		): BubbleSize {
			val ordinal = getInt(index, -1)
			return BubbleSize.entries.getOrNull(ordinal) ?: defaultValue
		}

		private fun findValidParent(view: View): ViewGroup? =
			view.ancestors.firstNotNullOfOrNull { p ->
