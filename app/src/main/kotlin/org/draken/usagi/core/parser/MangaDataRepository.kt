...
class MangaDataRepository
	...
	constructor(
		...
		suspend fun getReaderMode(mangaId: Long): ReaderMode? =
			db.getPreferencesDao().find(mangaId)?.let {
				ReaderMode.valueOf(it.mode)
			}
		...
		suspend fun cleanupLocalManga() {
			...
			val broken =
				dao
					.findAllBySource(LocalMangaSource.name)
					.filter { x ->
						x.manga.url
							.toUri()
							.toFileOrNull()
		...
		fun observeOverridesTrigger(emitInitialState: Boolean) =
			db.invalidationTracker.createFlow(
				tables = arrayOf(TABLE_PREFERENCES),
				emitInitialState = emitInitialState,
			)

		fun observeFavoritesTrigger(emitInitialState: Boolean) =
			db.invalidationTracker.createFlow(
		...
		private fun MangaPrefsEntity.getColorFilterOrNull(): ReaderColorFilter? =
			if (cfBrightness != 0f || cfContrast != 0f || cfInvert || cfGrayscale || cfBookEffect) {
				ReaderColorFilter(
		...
		private fun MangaPrefsEntity.getOverrideOrNull(): MangaOverride? =
			if (titleOverride.isNullOrEmpty() && coverUrlOverride.isNullOrEmpty() &&
		...
		private fun newEntity(mangaId: Long) =
			MangaPrefsEntity(
				mangaId = mangaId,
				mode = -1,
