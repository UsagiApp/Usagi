...
class MangaLinkResolver
	...
	constructor(
		...
		suspend fun resolve(uri: Uri): Manga =
			if (uri.scheme == "kotatsu" || uri.scheme == "usagi" || uri.host == "kotatsu.app" ||
				uri.host == "yumemi.moe"
			) {
				resolveAppLink(uri)
			} else {
		...
		private suspend fun MangaRepository.findExact(
			url: String?,
			title: String?,
		): Manga? {
			if (!title.isNullOrEmpty()) {
				...
				list
					.minByOrNull { it.title.levenshteinDistance(title) }
		...
		private suspend fun MangaRepository.getDetailsNoCache(manga: Manga): Manga =
			if (this is CachingMangaRepository) {
		...
		companion object {
			fun isValidLink(str: String): Boolean =
				str.isHttpUrl() || str.startsWith("kotatsu://", ignoreCase = true) ||
					str.startsWith("usagi://", ignoreCase = true)
		}
	}
