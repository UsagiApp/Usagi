	...
	constructor(
		...
		suspend fun getAvailableVersions(): List<AppVersion> {
			...
			return jsonArray.mapJSONNotNull { json ->
				val asset =
					json.optJSONArray("assets")?.find { jo ->
		...
		suspend fun fetchUpdate(force: Boolean = false): AppVersion? =
		...
		suspend fun isUpdateSupported(): Boolean = BuildConfig.BUILD_TYPE != BUILD_TYPE_RELEASE || appValidator.isOriginalApp.getOrNull() == true
