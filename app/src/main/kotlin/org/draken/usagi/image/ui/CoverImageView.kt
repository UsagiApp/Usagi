...
class CoverImageView
	...
	constructor(
		...
		fun setImageAsync(page: ReaderPage) =
			enqueueRequest(
				newRequestBuilder()
		...
		fun setImageAsync(page: MangaPage) =
			enqueueRequest(
				newRequestBuilder()
					.data(page)
					.mangaSourceExtra(page.source)
		...
		fun setImageAsync(bookmark: Bookmark) =
			enqueueRequest(
				newRequestBuilder()
		...
		private inner class ErrorForegroundListener : ImageRequest.Listener {
			...
			override fun onError(
				...
				foreground =
					if (result.throwable.isNetworkError() && !networkState.isOnline()) {
			...
			private fun Throwable.getShortMessage(): String? =
				when (this) {
