...
class SyncController
	...
	constructor(
		...
		override fun onInvalidated(tables: Set<String>) {
			val favourites =
				(TABLE_FAVOURITES in tables || TABLE_FAVOURITE_CATEGORIES in tables) &&
		...
		fun observeSyncStatus(): Flow<Boolean> =
			callbackFlow {
				val handle =
					ContentResolver.addStatusChangeListener(SYNC_OBSERVER_TYPE_ACTIVE) { which ->
		...
		private suspend fun requestSyncImpl(
			favourites: Boolean,
			history: Boolean,
		) = mutex.withLock {
			if (!favourites && !history) {
		...
		private suspend fun MangaDatabase.gc(
			favourites: Boolean,
			history: Boolean,
		) = withTransaction {
