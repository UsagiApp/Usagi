	...
	constructor(
		...
		private fun upsertManga(
			...
			for (tag in tags) {
				result +=
					ContentProviderOperation
						.newInsert(uri(authority, TABLE_TAGS))
						.withValues(tag.toContentValues())
						.build()
				result +=
					ContentProviderOperation
						.newInsert(uri(authority, TABLE_MANGA_TAGS))
						.withValues(
			...
			result.add(
				0,
				ContentProviderOperation
					.newInsert(uri(authority, TABLE_MANGA))
		...
		private fun getHistory(): List<HistorySyncDto> =
			provider.query(authorityHistory, TABLE_HISTORY).use { cursor ->
