...
class PagesViewModel
	...
	constructor(
		...
		private suspend fun doInit(state: State) {
			...
			val initialChapterId =
				state.readerState?.chapterId?.takeIf {
					chaptersLoader.peekChapter(it) != null
				} ?: state.details.allChapters
					.firstOrNull()
					?.id ?: return
		...
		private fun loadPrevNextChapter(isNext: Boolean): Job =
			launchJob(Dispatchers.Default) {
		...
		private fun updateList(readerState: ReaderState?) {
			...
			val pages =
				buildList(snapshot.size + chaptersLoader.size + 2) {
